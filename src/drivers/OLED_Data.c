...
/* 6x8 像素 ASCII 字库，索引 0~94 对应空格(0) 到 ~(94) */
...
/* 16x16 像素汉字字库。
 * 同一个字只需存储一份，按使用先后顺序排列。
 * 全部为汉字，不含全角字符和半角字符。
 */
const ChineseCell_t OLED_CF16x16[] = {

    /* 未找到指定字模时显示的默认图形（带问号的方框），放在末尾用于索引归零 */
    "",
